...
data_mult = {"mot1":[10, 11, 15, 12, 13, 18, 15, 100, 110, 150, 120, 130, 180, 150, 15, 14, 15,14, 18, 19, 20, 205, 260, 280, 14, 18, 16, 14, 25, 28, 31, 10, 11, 15, 12, 13, 18, 15, 100, 110, 150, 120, 130, 180, 150, 15, 14, 15,14, 18, 19, 20, 205, 260, 280, 14, 18, 16, 14, 25, 28, 31]}
data_test = [10, 11, 15, 12, 13, 18, 15, 100, 110, 150, 120, 130, 180, 150, 15, 14, 15,14, 18, 19, 20, 205, 260, 280, 14, 18, 16, 14, 25, 28, 31, 10, 11, 15, 12, 13, 18, 15, 100, 110, 150, 120, 130, 180, 150, 15, 14, 15,14, 18, 19, 20, 205, 260, 280, 14, 18, 16, 14, 25, 28, 31]
data_mult2 = {"mot1":[10, 11, 15, 12, 13, 18, 15, 100, 110, 150, 120, 130, 180, 150, 15, 14, 15,14, 18, 19, 20, 205, 260, 280, 14, 18, 16, 14, 25, 28, 31, 100, 110, 150, 120, 130, 180, 150, 1000, 1100, 1500, 1200, 1300, 1800, 1500, 150, 140, 150,140, 180, 190, 200, 2050, 2600, 2800, 140, 180, 160, 140, 250, 280, 310]}
...
def trend_by_period(data, interest_period):
    ...
    for  key, val in data.items():
        values_serie = val
        word = key
    res_additive = sm.tsa.seasonal_decompose(values_serie, freq = interest_period,
                                    model='additive')
    res_multiplicative = sm.tsa.seasonal_decompose(values_serie, freq = interest_period,
                                    model='multiplicative')
    ...
    if interest_period == 7:
        ...
        if sum(residual_additive) > 0 :
            if sum(cor_add**2) >= sum(cor_mult**2):
                print('additive')
                sum(cor_add**2)
                trend_values = {word:[res_additive.trend[3:6], res_additive.trend[7:10]]}
                test_trend = file_trend(trend_values)
                print(trend_values)
            else:
                print('mult')
                sum(cor_mult**2)
                trend_values = {word:[res_multiplicative.trend[3:6], res_multiplicative.trend[7:10]]}
                test_trend = file_trend(trend_values)
                print(trend_values)
        else:
        ...
        if sum(residual_additive) > 0 :
            ...
            if sum(cor_add**2) >= sum(cor_mult**2):
                ...
                trend_values = {word:[res_additive.trend[15:31], res_additive.trend[32:47]]}
                test_trend = file_trend(trend_values)
            else:
                print('mult')
                trend_values = {word:[res_multiplicative.trend[15:31], res_multiplicative.trend[32:47]]}
                test_trend = file_trend(trend_values)
        else:
            test_trend = {word:'Pas_de_Tendance'}
    return test_trend
